...
```
##Spatial
Since we have lat/long coordinates, its trivially easy to make map out this data. I originally had ambitions to make an animated map, but the result turned out to be less than compelling, so here I'll just map the heyday of post offices in 1900 and what was left as of 2000.  We'll pull County boundaries for context with the {USAboundaries} package.
```{r spatial}
...
ggplot(data=vt_counties) +
  geom_sf() +
  geom_sf(data = po_spatial, aes(color = established), alpha = .5) +
  theme_void() +
